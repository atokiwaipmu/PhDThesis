...
The two-point correlation function (2PCF), denoted by $\xi(\mathbf{r})$, is a fundamental statistical tool in cosmology that characterizes the clustering of matter in the Universe. It quantifies the excess probability of finding a pair of points separated by a distance $\mathbf{r}$ compared to a random distribution \citep{2002PhR...367....1B}, defined as:
\begin{equation}
    \xi(\mathbf{r}) = \langle \delta(\mathbf{x}) \delta(\mathbf{x} + \mathbf{r}) \rangle,
    \label{eq:2pcf}
\end{equation}
where $\delta(\mathbf{x})$ represents the matter density contrast at position $\mathbf{x}$, and $\langle \cdot \rangle$ denotes the ensemble average over all positions in the Universe. 

In cosmological analyses, the tracers of the underlying matter distribution, such as galaxies or clusters of galaxies, are often biased. This means their spatial distribution does not perfectly follow that of the dark matter. To accurately model the 2PCF of these biased tracers, the concept of halo bias is introduced (see Section~\ref{sec:halo}). For linear bias models, the relationship between the biased tracers and the underlying matter density contrast is expressed as:
\begin{equation}
    \delta_h(\mathbf{x}) = b_1 \delta(\mathbf{x}),
    \label{eq:bias}
\end{equation}
where $\delta_h(\mathbf{x})$ is the density contrast of the biased tracers, and $b_1$ is the linear bias parameter. Substituting Equation~\eqref{eq:bias} into Equation~\eqref{eq:2pcf}, the biased 2PCF, $\xi_h(\mathbf{r})$, becomes \citep{1996MNRAS.282..347M}:
\begin{equation}
    \xi_h(\mathbf{r}) = b_1^2 \xi(\mathbf{r}).
    \label{eq:bias_2pcf}
\end{equation}
This relation indicates that the clustering strength of biased tracers is amplified by the square of the bias parameter, $b_1^2$, compared to the underlying matter distribution. Therfore, understanding and accurately modeling this bias is crucial for interpreting observational data.
